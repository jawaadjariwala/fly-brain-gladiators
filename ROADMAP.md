...
- [x] Extract the working subgraph
  ...
  - **22,163 neurons (13.3%), 4.08M edges (16.6%)**: every seed retained
...
- [x] **Match serialisation** (`fbg/export.py`): one binary per match: a JSON header, then arrays laid out 8-byte aligned so each is a typed-array view in the browser with nothing parsed twice. Body state, animation state, per-tick firing rates, and the spikes. A typical match is about 0.9 MB, and the shared neuron layout 0.22 MB once
...
- [x] **Published** at [gladiators.jawaadjariwala.com](https://gladiators.jawaadjariwala.com), deployed from `web/` by GitHub Actions on every push
- [x] **Library**: 75 matches, every pairing at five seeds, 56 decisive to 19 drawn, mean 19.0 s
